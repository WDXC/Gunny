...
void Player::update(const unsigned int delta) {
  ...
  if (isMoving()) {
    ...
    prevPosDeltaX =
        posDiff * (isMovingHorizontally()
                       ? (movementDirection == MovementDirection::Left ? -1 : 1)
                       : 0);
    prevPosDeltaY =
        posDiff * (isMovingVertically()
                       ? (movementDirection == MovementDirection::Up ? -1 : 1)
                       : 0);
...
bool Player::isMovingHorizontally() const {
  return movementDirection == MovementDirection::Left ||
