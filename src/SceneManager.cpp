...
void SceneManager::addScene(const std::string &name,
  ...
  if (it != scenes.end() && it->second) {
    std::cout << "SceneManager::addScene - scene '" << name
              << "' already exist in scene tree!" << std::endl;
    return;
  }
...
void SceneManager::activateScene(const std::string &name) {
  ...
  if (it != scenes.end()) {
    ...
    std::cout << "SceneManager::activateScene - scene '" << name
...
void SceneManager::draw() const { currentScene->draw(); }
